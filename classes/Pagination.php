...
class Pagination {
	...
	public function url($page = 1)
	{
		...
		switch ($this->config['current_page']['source'])
		{
			case 'query_string':
				...
				return URL::site($uri) . URL::query([$this->config['current_page']['key'] => $page]);

			case 'route':
				return URL::site(Request::current()->uri([$this->config['current_page']['key'] => $page])) . URL::query();
		}
	...
	public function render($view = NULL)
	{
		...
		if (!$view instanceof \Kohana\Core\View)
		{
			// Load the view file
